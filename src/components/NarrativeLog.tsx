    ...
    return (
        <div className="bg-red-900/50 border border-red-700 p-3 rounded-lg">
            <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-sm text-slate-300 font-mono">{enemy.hp} / {enemy.maxHp} HP</span>
            </div>
...
export const NarrativeLog: React.FC<NarrativeLogProps> = ({ log, enemies, playerName, logAnimStartIndex }) => {
  ...
  return (
    <div className="p-4 space-y-4 h-full">
        {enemies.length > 0 && (
            <div className="p-4 border-2 border-red-600 bg-slate-800/50 rounded-lg">
                <h3 className="text-xl font-bold font-serif text-red-500 mb-3 text-center animate-pulse tracking-widest">!!! COMBAT !!!</h3>
      ...
      <div className="space-y-4 text-slate-300">
        {log.map((entry, index) => {
          ...
          if (typeof entry === 'string') {
            return (
              <p key={index} className={`text-sm italic text-amber-300 pl-4 border-l-2 border-amber-500/50 ${animationClass}`}>
                {entry}
              </p>
            );
          }
          if (entry.type === 'player') {
            return (
              <div key={index} className={`flex justify-end ${animationClass}`}>
                <p className="bg-sky-900/70 text-slate-200 rounded-lg rounded-br-none py-2 px-4 max-w-lg italic shadow">
                    &gt; {entry.content}
                </p>
              </div>
            );
          ...
          if (entry.type === 'combat') {
            return (
              <p key={index} className={`text-center font-bold text-red-400 py-2 my-2 border-y-2 border-red-700/50 bg-red-900/30 font-serif tracking-wider ${animationClass}`}>
                ⚔️ {entry.content} ⚔️
              </p>
            );
          }
